...
// Image
$attrs_image['class'][] = $params['image_align'] == 'top' ? 'uk-margin-small-bottom' : 'uk-margin-medium-bottom';

// Header
$attrs_header['class'][] = 'uk-margin-small-bottom';
...
// Button
$attrs_button['class'][] = "uk-text-uppercase uk-text-bold uk-button uk-button-{$params['button_style']}";
$attrs_button_container['class'][] = $params['header_align'] ? 'uk-text-center' : '';
$attrs_button_container['class'][] = 'uk-margin';
        ...
        switch ($params['meta_style']) {
            ...
            default: // sentence
                ...
                if ($author && $date) {
                    ...
                    printf(__('<span class="fa fa-clock-o" aria-hidden="true"></span>
 %s', 'yootheme'), get_the_time('M j, Y'));
                }
...
<article id="post-<?php the_ID() ?>" <?php post_class('uk-article') ?> typeof="Article">
	...
	<div class="uk-grid uk-grid-medium">
		...
		<div class="uk-width-2-3@m">
			...
			<div class="post-content" <?= get_attrs($attrs_content) ?> property="text"><?php //echo wp_trim_words( get_the_content(), $num_words = 40, ''); ?>
			<?php $cat = get_the_content();
			$cat = preg_replace('/<img[^>]+./','', $cat);
			echo html_cut($cat,150);
			?>
			</div>

			<p<?= get_attrs($attrs_button_container) ?>>
				<a<?= get_attrs($attrs_button) ?> href="<?= get_permalink() ?>">Read More</a>
			</p>
